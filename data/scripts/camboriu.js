...
document.querySelectorAll('tbody tr').forEach(row => {
    ...
    const lattesContainer = row.querySelector('td:nth-child(3) a')
    if (lattesContainer)
        professor.lattes = lattesContainer.href

    const riaContainer = row.querySelector('td:nth-child(6) a')
    if(riaContainer)
        professor.ria = riaContainer.href
    
    const ptdContainer = row.querySelector('td:nth-child(7) a')
    if (ptdContainer)
        professor.ptd = ptdContainer.href
    
    const horarioContainer = row.querySelector('td:nth-child(8) a')
    if (horarioContainer)
