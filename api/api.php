...
class IFCAPI{
    ...
    function getCampusInfo($KEYS){
        try{
            $response = $this->connection->prepare(
                'SELECT cidade_campus, telefone_campus, site_campus FROM campus
                WHERE id_campus = ?');
            $response->bindParam(1, $KEYS['idcampus']);
            $response->execute();
    ...
    function getProfessor($KEYS){
        try{
            ...
            $query = "SELECT p.nome_professor, p.email_professor, p.lattes_professor, h.link_horario FROM professor p
